...
async function sticker(img, url, ...args) {
    ...
    // packname y author vienen de args[0] y args[1]
    let packname = args[0] || global.packname
    let author = args[1] || global.author
    ...
    for (let func of [
        ...
        try {
            stiker = await func(img, url, packname, author, ...args.slice(2))
            if (stiker.includes('html')) continue
            if (stiker.includes('WEBP')) {
                try {
                    return await addExif(stiker, packname, author, ...args.slice(2))
                } catch (e) {
                    console.error(e)
                    return stiker
                }
