...
// ====================================================================
// El bot asume que está sirviendo a la red 'alastor'

// Endpoints para Códigos de Verificación
const CODES_API_ENDPOINT = 'https://report-bots-causas.duckdns.org/api/verification/codes/pending/alastor'; 
const CODES_NOTIFICATION_BASE_URL = 'https://report-bots-causas.duckdns.org/api/verification/codes/mandado/alastor'; 

// Endpoints para Mensajes del Bot (Actualizaciones de Reportes)
const MESSAGES_API_ENDPOINT = 'https://report-bots-causas.duckdns.org/api/verification/messages/pending/alastor';
const MESSAGES_NOTIFICATION_BASE_URL = 'https://report-bots-causas.duckdns.org/api/verification/messages/mandado/alastor';

const TARGET_NETWORK = 'ALASTOR'; 

// ====================================================================
// CHEQUEO UNIFICADO Y SECUENCIAL DE ENDPOINTS
// ====================================================================

/**
 * Realiza una revisión SILENCIOSA, priorizando códigos de verificación (si hay, termina aquí),
 * y luego (si no hay códigos) revisando mensajes de actualización de reportes.
 * @param {object} conn - La instancia de conexión de Baileys.
 * @param {object} dbData - La data de la base de datos (global.db.data).
 */
export async function checkPendingEndpoints(conn, dbData) {
    
    const botUsers = dbData.users || {};
    let codesWereProcessed = false;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (pendingCodes && pendingCodes.length > 0) {
                ...
                // Si encontramos códigos, marcamos para evitar la revisión de mensajes.
                ...
                if (sentCodeIds.length > 0) {
                    ...
                    try {
                        ...
                        // 🛑 CORRECCIÓN CLAVE: Fuerza error si la API no confirma.
    ...
    // ----------------------------------------------------
    // VERIFICACIÓN CONDICIONAL: Si se procesaron códigos, terminamos aquí.
